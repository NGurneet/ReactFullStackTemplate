...
export const saveRefreshToken = async (userId: string, token: string): Promise<any> => {
  const user = await UserSchema.findByIdAndUpdate(userId, { refreshToken: token }, { new: true });
  return user;
};
...
export const verifyRefreshToken = async (token: string): Promise<any> => {
  try {
    ...
    const user = await UserSchema.findOne({ _id: decoded._id, refreshToken: token });
...
export const deleteRefreshToken = async (userId: string): Promise<any> => {
  const user = await UserSchema.findByIdAndUpdate(userId, { refreshToken: "" }, { new: true });
  return user;
};
